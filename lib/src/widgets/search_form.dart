...
import 'package:test_app/src/models/search_result_item.dart';

class SearchForm extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Column(
      children: <Widget>[SizedBox(height: 10.0),_SearchBar(),SizedBox(height: 10.0), _SearchBody()],
    );
  }
}
    ...
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        border: OutlineInputBorder(
          //TODO: check the Radius int value
  ...
  Widget build(BuildContext context) {
    return ListTile(
      leading: CircleAvatar(
        child: Image.network(item.user.name),
      ),
      title: Text(item.name),
      onTap: () {
            City city = City(item.id, item.name);
